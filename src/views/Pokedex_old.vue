...
async function loadPokemon() {
  try {
    const data = await dataController.getPokemonList(currentPage.value, itemsPerPage);
    const typedData = data as { pokemon: { name: string; sprite: string }[] }; // Cast 'data' to the expected type
    pokemonList.value = typedData.pokemon; // Lista de Pokémon
    //totalPages.value = data.totalPages; // Total de páginas
  } catch (error) {
